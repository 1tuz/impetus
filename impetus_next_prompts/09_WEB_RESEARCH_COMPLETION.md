# Довести Web Research до законченного research loop и capability model

Низкоуровневую SSRF-защиту не переписывать без причины. Сначала проверить существующую реализацию initial URL/DNS/redirect/final destination и сохранить её сильные свойства.

## Цель

Закрыть вертикаль:
`search → select → fetch → follow links → compare → cite`.

## Реализовать/проверить

- bounded multi-step research loop;
- dedup источников;
- provenance/citations от search result до final answer;
- ограничение числа fetch/follow steps;
- artifacts для больших страниц;
- deterministic source ranking/selection;
- timeout/budget/cancellation;
- fallback SearchBackend health;
- doctor diagnostics.
...
- `web.submit`;
- `web.upload`;
- отдельный explicit capability для LAN/internal targets.

Read-only public web не должен автоматически разрешать POST/upload/auth actions.

Любой outbound data path должен проходить более строгую Policy/approval.

## Browser

BrowserProvider остаётся optional module. Не делай Chromium/Playwright/Node обязательной зависимостью harness.

## Acceptance criteria

- Агент может выполнить ограниченное research-задание и вернуть ответ с provenance.
- Redirect/DNS rebinding/private target tests остаются зелёными.
- POST/upload нельзя получить через обычный `web.read`.
- Doctor показывает per-backend health и degraded fallback.

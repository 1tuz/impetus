# Reference Store: сделать общим механизмом, а не продуктовой привязкой к одному примеру

Reference Store должен быть generic long-term reference context. Конкретные системы учёта/трекеры — только optional import adapters/examples, а не архитектурная цель core.

## Цель

Сохранить YAML как человекочитаемый authoritative format для умеренных datasets, но убрать ложную «YAML как эффективная БД для всего» модель.

## Улучшения

Проверь/реализуй:
- generic dataset manifest;
- schema version;
- sensitivity/scope/provenance;
- generic importer interface;
- optional adapters не проникают в core contracts;
- actual calendar partitioning:
  - monthly = настоящий `YYYY-MM`;
  - yearly = настоящий calendar year;
  - не `timestamp / 30 days` и не `365 days from epoch`;
- derived local index для быстрого поиска, который можно полностью rebuild из YAML;
- SQLite FTS/Tantivy/другой лёгкий Rust-friendly индекс — выбери по текущим зависимостям и YAGNI;
- Top-K сначала по индексу, затем lazy load только нужных record bodies;
- фильтры project/time/tags/sensitivity;
- atomic writes и corruption recovery;
- benchmark на dataset, достаточно большом, чтобы отличить scan-all от indexed lookup.

## Acceptance criteria

- Поиск не читает все YAML partitions на каждый query.
- YAML остаётся authoritative/rebuildable.
- Core не зависит от конкретного трекера/продукта.
- Calendar partition tests проходят на границах месяца/года/leap year.
- Derived index можно удалить и полностью восстановить.

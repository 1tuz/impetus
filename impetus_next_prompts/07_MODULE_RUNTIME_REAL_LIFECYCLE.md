# Module Runtime: доказать реальный lifecycle вместо scaffold

TODO/checkmarks не являются доказательством. Проверь `discover/probe/start/health/stop` по production пути.

## Цель

Module Runtime должен реально управлять модулем, а не только менять enum состояния.

Проверь/реализуй:
- discovery реальных registered/external modules;
- capability probing, основанный на runtime reality, а не `available: true`;
- version/platform/contract compatibility;
- start внешнего модуля как отдельного process + versioned IPC;
- sandbox/isolation согласно declared permissions;
- health probe с degraded/unavailable semantics;
- graceful stop + forced termination;
- crash detection;
- restart policy только там, где semantics безопасны;
- `UnknownOutcome` запрет fallback/retry для mutating/non-replayable;
- safe fallback per module kind;
- ServiceRegistry/ModuleRegistry не создают обход Policy.

Не вводи arbitrary dynamic-library ABI/marketplace.

## Acceptance criteria

- Есть хотя бы один production-like external test module, проходящий полный lifecycle.
- `discover()` не возвращает пустой placeholder без причины.
- `probe()` способен вернуть unavailable/incompatible/degraded.
- `start()` реально создаёт runtime/process там, где descriptor external.
- `health()` отражает падение процесса.
- `stop()` реально завершает runtime.
- Tests доказывают отсутствие policy bypass при degraded/fallback path.

# Удалить концепцию local-agent → cloud research adviser

Пользователь отказался от отдельной архитектурной идеи, в которой маленькая локальная модель сама решает, что задача сложная, формирует sanitised research request и вызывает более мощную облачную модель, после чего продолжает работу локально.

Нужно убрать **именно эту концепцию** из Impetus как продуктового требования и из кода, если под неё существует отдельная логика.

## Что сохранить

Сохранить нормальный Model Router:
- выбор provider/model по capability, health, cost, latency, privacy, context, cache и budget;
- обычный fallback/retry между провайдерами при технической недоступности;
- `UnknownOutcome` правила для mutating/non-replayable действий;
- local-first/free-first/balanced/quality-first как обычные политики выбора, если они полезны сами по себе.

Не удалять обычный routing только потому, что в нём есть локальные и облачные модели.

## Что убрать

Проверь и убери:
- формулировки `local → sanitised cloud request → result back to local agent`;
- отдельные `ResearchPacket`/complexity-escalation/self-judging механизмы, если они существуют только ради этой идеи;
- TODO/ROADMAP/ARCHITECTURE заявления, будто эта схема является целевой архитектурой или завершённой функцией;
- тесты/конфиги, существующие исключительно для этого поведения.

Если `escalation_chain` — всего лишь общий fallback chain, не удаляй его автоматически. Переименуй/уточни семантику, если название провоцирует неверное понимание.

## Acceptance criteria

- В canonical docs больше нет product requirement на self-escalation локальной модели в облачную «модель-советника».
- Обычный model routing/fallback остаётся работоспособным.
- Нет мёртвого кода, оставшегося только от удалённой идеи.
- Тесты явно фиксируют границу между обычным provider fallback и удалённой research-escalation концепцией.
- TODO/ROADMAP отражают фактическое состояние, без ложного `[x]`.

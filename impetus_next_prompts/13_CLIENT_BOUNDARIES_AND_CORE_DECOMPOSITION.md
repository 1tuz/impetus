# Очистить архитектурные границы клиентов и раздробить god-dispatcher

## Часть A — Zap/client boundary

Проверь, не зависит ли Zap adapter напрямую от `impetus-core` ради `Event`, `IpcRequest`, `IpcResponse`, `RuntimeStatus` или других wire DTO.

Цель:
- Zap/TUI/будущие клиенты зависят от `impetus-client` + публичного versioned protocol crate/API;
- authoritative core types не становятся клиентским API случайно;
- transport/DTO compatibility явно versioned;
- никаких SQLite/policy/runtime internals в клиенте.

Если нужен отдельный `impetus-protocol` crate — вводи только если это уменьшает реальную связанность; не делай crate split ради эстетики.

## Часть B — `harness_api` decomposition

Если центральный dispatcher по-прежнему огромный и передаёт десятки аргументов:
- раздели handlers по bounded domains: sessions, runs, approvals, tools, context, diagnostics/components;
- введи state/context object вместо `too_many_arguments`, если это действительно улучшает ownership;
- убери operational `expect()`/panic paths, которые могут уронить daemon из-за recoverable storage/runtime error;
- сохранить единый Kernel safety/durability pipeline.

## Acceptance criteria

- Thin clients не импортируют core authoritative runtime types.
- Public protocol API имеет ясную ownership/version boundary.
- Recoverable request failure возвращает typed error, а не падает daemon.
- Refactor не меняет durable semantics и покрыт regression tests.

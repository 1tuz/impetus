...
show_post_install() {
  ...
  info ""
  info "Установлены binary:"
  info "  impetusd       — authoritative daemon (runtime, policy, execution)"
  info "  impetus        — user CLI client"
  info ""
  info "Добавьте $INSTALL_DIR в PATH, если ещё не добавлен:"
  info "  export PATH=\"\$HOME/.local/bin:\$PATH\""
  info ""
  info "Запуск (daemon стартует сам при первой команде):"
  info "  impetus doctor"
  info "  impetus create"
  info "  impetus ui"
  info ""
  info "Ручной impetusd — только для debug / --provider-profile / --acp-profile."
  info ""
  info "Удаление:"
  info "  rm -f ~/.local/bin/{impetus,impetusd}"
  info "  rm -rf ~/Library/Application\\ Support/Impetus  # macOS"
  info "  rm -rf ~/.local/share/impetus                 # Linux default"
  info ""
  info "Документация: https://github.com/$REPO/blob/main/README.md"
}

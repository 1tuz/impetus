...
| Legacy OpenAI-compatible text stream | Implemented | `openai_compat_adapter.rs` (still in tree; not default); `GET /v1/models` discovery on ListModels; parse optional metadata into catalog when present (incl. `service_tiers`); id-only → honest empty reasoning_efforts / service_tiers / capabilities (never invent low/medium/high); secret-shaped keys stripped from `provider_options` (#322 / #328) |
| Session model override → HTTP stream | Implemented | `SetSessionModel` → durable `$IMPETUS_DATA_DIR/session_models/{uuid}.json` + RAM cache → `StreamOptions` (`model` / `reasoning_effort` / `service_tier` / non-secret `provider_options`); reasoning + service_tier validated against **selected model** catalog (free-form; no global enum; tier only when advertised). Load/prompt/**RAM-hit**/FollowUp/approval-resume revalidate fail-closed (invalid RAM entry dropped). Options merge into OpenAI-compat/native request body; secrets rejected/stripped; CloseRouter metadata stays adapter `provider_options` JSON (#322 / #328) |
...
| Browser provider | Partial | Daemon IPC `GetBrowserHealth` / `NegotiateBrowser`: Active `BrowserIntegration` host_process operate preferred (#362); else honest Absent. Library Mock/Absent + Firefox/Chrome identity. Concrete CDP/WebDriver **Parked** in extensions — not core (#363). |
| Coding tools (definition/refs/diagnostics/symbols/hover/cancel) | Partial | Seam + IPC coding_* (#336). Daemon prefers Active `LspIntegration` operate (`PreferExtensionCodingTools`, #362); fallback `ProcessLspBackend` when `IMPETUS_LSP_BINARY` / `rust-analyzer` on PATH; else Unavailable. Crash respawn + cancel + diagnostics cache shipped. Full LSP protocol Parked; language packs in extensions (#363). |
